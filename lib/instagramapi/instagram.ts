  ...
  const igposts  = []
    ...
    for (let i = 0; i < 9; i++) {
      const post: IinstragramPost = {
        ...
        mediaType: feed.data[i].media_type,
        mediaUrl: feed.data[i].media_url,
        permalink: feed.data[i].permalink,
        timestamp: feed.data[i].timestamp,
        username: feed.data[i].username,
      }
      igposts.push(post)
    }
	} catch (error) {
